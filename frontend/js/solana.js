...
const MINT_DEV_BADGE_DISCRIMINATOR = new Uint8Array([102, 221, 55, 250, 237, 112, 154, 119])
const MINT_REPO_BADGE_DISCRIMINATOR = new Uint8Array([43, 60, 10, 231, 230, 125, 118, 14])
const UPDATE_DEV_BADGE_DISCRIMINATOR = new Uint8Array([115, 186, 3, 77, 214, 118, 70, 16])
const UPDATE_REPO_BADGE_DISCRIMINATOR = new Uint8Array([74, 96, 123, 10, 162, 151, 119, 226])
const VOUCH_FOR_DEV_DISCRIMINATOR = new Uint8Array([155, 55, 131, 60, 112, 41, 101, 7])

const CONNECTION = new solanaWeb3.Connection(
  "https://devnet.helius-rpc.com/?api-key=001a32fa-4613-48a3-8fb6-44d3dc1ee3a2",
  "confirmed",
)
...
window.buildAndSendRepoBadgeTx = async function (
  ...
  const [devStatePda] = findPda([toBytes("dev_state"), walletPubkey.toBytes()])
  const [devBadgePda] = findPda([toBytes("dev_badge"), walletPubkey.toBytes()])
  const [repoStatePda] = findPda([toBytes("repo_state"), devBadgePda.toBytes(), new Uint8Array(repoNamePadded)])
  const [repoBadgePda] = findPda([toBytes("repo_badge"), devBadgePda.toBytes(), new Uint8Array(repoNamePadded)])
  ...
  const totalLen = 8 + 32 + 32 + 4 + 4 + 4 + 4 + 1 + lang1Bytes.length + lang2Bytes.length
...
window.buildAndSendUpdateRepoBadgeTx = async function (
  ...
  const totalLen = 8 + 32 + 32 + 4 + 4 + 4 + 4 + lang1Bytes.length + lang2Bytes.length
  const ixData = new Uint8Array(totalLen)
  let offset = 0
...
window.buildAndSendVouchTx = async function (targetAddr) {
  ...
  // Derive PDAs
  const [ghostConfigPda] = findPda([toBytes("ghost_config")])
  const [voucherDevStatePda] = findPda([toBytes("dev_state"), walletPubKey.toBytes()])
  const [targetDevStatePda] = findPda([toBytes("dev_state"), new Uint8Array(targetAddr)])
  const [vouchRecordPda] = findPda([toBytes("vouch_record"), walletPubKey.toBytes(), new Uint8Array(targetAddr)])
